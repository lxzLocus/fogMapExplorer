...
3. **環境変数を設定**（Vercel ダッシュボード → Settings → Environment Variables、または `vercel env add`）:
   ...
   Vercel の **Storage → Postgres** を接続した場合は `POSTGRES_URL`（や `POSTGRES_URL_NON_POOLING`）が自動で入り、アプリはそれも自動的に読みます（`DATABASE_URL` / `POSTGRES_URL_NON_POOLING` / `POSTGRES_URL` / `POSTGRES_PRISMA_URL` の順に採用）。→ その場合は **`JWT_SECRET` を足すだけ**でOK。
4. **デプロイ**:
   ```bash
   vercel deploy --prod
   ```
